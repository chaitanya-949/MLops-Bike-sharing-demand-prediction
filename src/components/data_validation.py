...
class DataValidation:
    ...
    def is_column_exist(self, df: DataFrame) -> bool:
        ...
        try:
            ...
            for column in self._schema_config["object_columns"]:
                if column not in dataframe_columns:
                    missing_object_columns.append(column)


            if len(missing_object_columns)>0:
                logging.info(f"Missing object column: {missing_object_columns}")


            return False if len(missing_categorical_columns)>0 or len(missing_numerical_columns)>0 or len(missing_object_columns)>0 else True
        except Exception as e:
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            report_dir = os.path.dirname(self.data_validation_config.data_validation_file_path)
            ...
            with open(self.data_validation_config.data_validation_file_path, "w") as report_file:
